<template>
  ...
  <el-drawer title="系统设置" v-model="drawer" size="300px" :show-close="false" direction="rtl">
    <h3>整体风格设置</h3>
    <div class="theme-box">
      <theme-icon v-model:active="state.style" v-for="(row, index) in style" :key="index" :name="index" :tip="row.name"
        :logo="row.logo.background" :menu="row.menu.background" :header="row.header.background"
        :main="row.container.background" :activeColor="row.page.color"></theme-icon>
    </div>
    <h3>主题色</h3>
    <div class="theme-box">
      <theme-color v-for="(item, key) in themeColorArr" v-model:active="state.primaryColor"
        v-model:activeTextColor="state.primaryTextColor" :key="key" :color="item.color" :textColor="item.textColor"
        :tip="item.tip"></theme-color>
    </div>
    <h3>其他设置</h3>
    <div class="list">
      <div class="list-item" v-for="option in options" :key="option.name">
        <span>{{ option.name }}</span>
        <el-switch v-model="option.value" active-color="#13ce66" inactive-color="#ff4949" @change="change(option)">
        </el-switch>
      </div>
    </div>
  </el-drawer>
...
export default defineComponent({
  ...
  setup() {
    ...
    const state: State = reactive({
      style: appStore.theme.style,
      primaryColor: appStore.theme.primaryColor,
      primaryTextColor: '',
      menuType: appStore.theme.menuType
    })
    const themeColorArr = [
    ...
    watch(state, (newVal) => {
      ...
      Object.assign(appStore.theme, state)
      setTheme()
    })
    ...
    const options = reactive([
      ...
      { name: '保持一个菜单展开', value: appStore.expandOneMenu, store: 'expandOneMenu' }
    ])
